...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if($username == '' || $password == ''){
		...
		$check_username = "SELECT admin_name FROM admin WHERE admin_name='$username'";
		
		$result_username = mysqli_query($connection, $check_username);
		
		$confirm_username = mysqli_fetch_assoc($result_username);
		
		if($confirm_username['admin_name'] !== $username){
			...
			$check_password = "SELECT admin_name,password FROM admin WHERE admin_name='$username' AND password='$encrypt_pass' LIMIT 1";
			...
			if($confirm_password['password'] != $encrypt_pass){
				...
				$checklogin = "SELECT admin_name,password FROM admin WHERE admin_name='$username' AND password='$encrypt_pass' LIMIT 1";
		...
		if(mysqli_num_rows($result_login) == 1){
			...
			$userlogin = "SELECT admin_name FROM admin WHERE admin_name='$username'";
